...
1.  Дополнительные десятичные разряды для **Kilometers** и **Fuel Used** бесполезны. Используйте аргумент `"{:.2f}".format`. Для форматирования значений с плавающей запятой до двух десятичных знаков перед преобразованием их в строковые значения, как показано ниже. Каждый оператор должен быть на отдельной строке.
