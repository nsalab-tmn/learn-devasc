...
Обратите внимание, что Ansible показывает, что файл инвентаризации **hosts**, который он будет использовать по умолчанию, - это **/etc/ansible/hosts**. На предыдущем шаге вы отредактировали файл инвентарных **hosts** в каталоге **ansible-csr1000v directory**. На следующем шаге вы отредактируете новый файл **ansible.cfg**, который использует созданный вами файл инвентаризации **hosts**.
...
-   Вы отредактировали файл **ansible.cfg**, чтобы использовать файл локальных хостов в качестве файла инвентаризации (**inventory=./hosts**).
    ...
    В следующей части вы создадите плейбук, чтобы сообщить Ansible, что делать.

### Часть 3. Используйте Ansible для резервного копирования конфигурации

В этой части вы создадите сценарий Ansible, который автоматизирует процесс резервного копирования конфигурации CSR1000v. В основе Ansible - плейбуки (playbook). Если вы хотите, чтобы Ansible получил информацию или выполнил действие на устройстве или группе устройств, вы запускаете плейбук, чтобы выполнить задание.

Плейбук Ansible - это файл YAML, содержащий одно или несколько воспроизведений. Каждый плейбук - это набор заданий (task).
...
-   Плейбука, который вы хотите запустить (**backup_cisco_router_playbook.yaml**)
-   Файла инвентаризации и его расположения (**-i hosts**).
...
Созданный вами плейбук содержит один плей (play) с двумя задачами (task). Ниже приводится объяснение вашего плейбука:
...
-   `name: AUTOMATIC BACKUP OF RUNNING-CONFIG` - Это название плейбука.
-   `hosts: CSR1kv` - Это псевдоним ранее настроенного файла **hosts**. Ссылаясь на этот псевдоним в вашем плейбуке, он может использовать все параметры, связанные с этой записью файла инвентаризации, включая имя пользователя, пароль и IP-адрес устройства.
...
1.  Еще раз проверьте файл инвентаризации **hosts**. Напоминаем, что этот файл содержит псевдоним **CSR1kv** и три инвентарные переменные для имени пользователя, пароля и IP-адреса хоста. В сборнике пьесплейбуке для этой части также будут использоваться этот файл и файл **ansible.cfg**, который вы создали в начале лабораторной работы.
...
В остальном плейбук аналогичен задачам из предыдущей части. Вторая задача использует модуль **ios_command** и команду **show ipv6 interface brief** для отображения вывода и отправки его на вывод регистра.
